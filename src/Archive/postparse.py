...
def unpack(logString, supplyString):
    ...
    for line in lines:
        ...
        players = [int(x) - 1 for x in rawPlayers.split("/")] if rawPlayers else []
        items = [unpackCardstack(x) for x in rawItems.split("/")] if rawItems else []
...
def get_vps(state, kingdom):
    ...
    for player in range(len(state["DECKS"])):
        ...
        entries = [
            [0, Cards[card].worth(state, player), Cards[card].index]
            for card in playerDeck
            if Cards[card].worth(state, player) != 0
        ]
        entries += [
            [1, Cards[card].worth(state, player), Cards[card].index]
    ...
    return (
        [
            "/".join(["|".join([str(x) for x in entry]) for entry in playerLine])
