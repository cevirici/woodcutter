...
def vineyard_worth(state, player):
    ...
    estates = playerDeck["ESTATE"] if state.inherited[player] != "NOTHING" else 0
    return (
        sum([playerDeck[item] for item in playerDeck if "a" in Cards[item].types])
        + estates
    ) // 3
...
def silkroad_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    return (
        sum([playerDeck[item] for item in playerDeck if "v" in Cards[item].types]) // 4
    )
...
def distantlands_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    return state["OTHERS"][player]["DISTANT LANDS"] / playerDeck["DISTANT LANDS"]


def humblecastle_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    return sum([playerDeck[item] for item in playerDeck if "c" in Cards[item].types])


def kingscastle_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    return (
        sum([playerDeck[item] for item in playerDeck if "c" in Cards[item].types]) * 2
    )
...
def keep_worth(state, player):
    ...
    return 5 * len(
        [
            ...
            if "t" in Cards[card].types and playerDeck[card] >= otherDeck[card]
        ]
    )
...
def obelisk_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    return 2 * sum([playerDeck[card] for card in playerDeck if card in state.obelisk])


def orchard_worth(state, player):
    playerDeck = state.crunch(GameState.playerZones, [player])
    estates = (
        4 if state.inherited[player] != "NOTHING" and playerDeck["ESTATE"] >= 3 else 0
    )
    return (
        len(
            [
                item
                for item in playerDeck
                if "a" in Cards[item].types and playerDeck[item] >= 3
            ]
        )
        * 4
        + estates
...
def tower_worth(state, player):
    def card_worth(card, state):
        ...
        if (
            "v" not in Cards[card].types and Cards[card].supply_type in [0, 1]
        ) or card == "DAME JOSEPHINE":
            for pair in pairs:
                if card in pair:
                    for others in pair:
    ...
    playerDeck = state.crunch(GameState.playerZones, [player])
    return sum([card_worth(card, state) * playerDeck[card] for card in playerDeck])
