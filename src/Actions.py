    ...
    def out_function(moves, i, blockLength, state):
        ...
        def innovation_check(move):
            return move.pred == 'SET ASIDE' and \
                ...
                move.items[0].primary == target.primary
    ...
    triggers = {'ARTISAN': [gainTo('SUPPLY', 'HANDS')],
                ...
                'GOLEM': [Exception(check(['REVEAL']),
                          ...
                          Exception(check(['PLAY']), move_play('OTHERS'),
                                    persistent=True),
                          checkMove(['DISCARD'], 'OTHERS', 'DISCARDS')],
                'SCRYING POOL': [persistent(exc_revealTopdeck),
                ...
                'DUCHESS': [persistent(exc_revealTopdeck),
                            persistent(exc_revealDiscard)],
                ...
                'ORACLE': [persistent(exc_revealTopdeck),
                           persistent(exc_revealDiscard)],
                ...
                'TRANSMOGRIFY': [checkMove(['PUT ONTO'],
                                           'INPLAYS', 'TAVERN')],
                'WARRIOR': [persistent(exc_revealDiscard),
                            persistent(checkMove(['TRASH'],
                                                 'DISCARDS', 'TRASH'))],
