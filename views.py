...
def display(request, game_id, logIndex=0):
    ...
    pairStr = "~".join(
        [
            "|".join([str(Cards[card].index) for card in [pairs[cards]] + list(cards)])
            for cards in pairs
        ]
    )
    ...
    kingdomStr = [[str(Cards[card].index) for card in row] for row in kingdomRaw]
    turnStates = [gameStates[i] for i in turnPoints]
    turnDecks = [
        "/".join(
            [repr(state.crunch(GameState.playerZones, [player])) for player in range(2)]
        )
        for state in turnStates
    ]
    ...
    context = {
        ...
        "scores": "~".join([" ".join(scores[point][0]) for point in turnPoints]),
        "scoreTotals": "~".join(["|".join(score[1]) for score in scores]),
...
def find_logs(request):
    ...
    searchCardIndices = [
        cleanNames.index(clean(cardName))
        for cardName in searchCards
        if clean(cardName) in cleanNames
    ]

    optionalCards = request.POST["optionals"].split(",")
    optionalCardIndices = [
        cleanNames.index(clean(cardName))
        for cardName in optionalCards
        if clean(cardName) in cleanNames
    ]
    ...
    def matchLog(rawLog, search, optional, players, errors):
        ...
        if len([card for card in optionalCardIndices if card in logCards]) < min(
            len(optionalCardIndices), 1
        ):
            return False

        logPlayers = rawLog.players.split("~")
        if len([player for player in players if player in logPlayers]) < min(
            len(players), 1
        ):
            return False

        if (errors == "0" and not rawLog.valid) or (errors == "2" and rawLog.valid):
            return False

        return True

    outputLogs = [
        '<a href = "{}/display">{}</a>'.format(rawLog.game_id, rawLog.game_id)
        for rawLog in rawLogs
        if matchLog(rawLog, searchCards, optionalCards, players, errors)
    ]
